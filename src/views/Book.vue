...
export default {
  ...
  created(){
    ...
    try {
      const resp = await axios.get(`https://www.googleapis.com/books/v1/volumes/${book_id}`);
...
<template>
<div v-if="book" class="mx-16">
  <br><br>
  <template v-if="book">
    <img :src="book.volumeInfo.imageLinks.thumbnail" :alt="book.volumeInfo.title" class="border border-gray-900">
  </template>
  <template v-else>
    <img
  ...
  <p class="font-mono"> {{book.volumeInfo.description}}</p>

  <button class="text-sm items-center mx-2 my-2 bg-blue-600 hover:bg-blue-800 text-white font-bold py-2 px-2 rounded">Add Book</button>
